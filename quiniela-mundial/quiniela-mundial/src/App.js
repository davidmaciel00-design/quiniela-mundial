...
const KO_ROUNDS = [
  ...
  {id:"ro16",label:"Octavos de Final",matches:8},
  {id:"qf",label:"Cuartos de Final",matches:4},
  {id:"sf",label:"Semifinales",matches:2},
  {id:"3rd",label:"Tercer Puesto",matches:1},
  {id:"final",label:"Gran Final",matches:1},
];

// KO matches are dynamic — defined by admin in Supabase under "ko_config"
// ko_config = { ro32: { enabled, matches: [{id,home,away}] }, ro16: {...}, ... }
const emptyKoConfig = () => ({
  ro32:  {enabled:false, matches: Array.from({length:16},(_,i)=>({id:`ro32_${i+1}`,home:"",away:""}))},
  ro16:  {enabled:false, matches: Array.from({length:8}, (_,i)=>({id:`ro16_${i+1}`,home:"",away:""}))},
  qf:    {enabled:false, matches: Array.from({length:4}, (_,i)=>({id:`qf_${i+1}`,home:"",away:""}))},
  sf:    {enabled:false, matches: Array.from({length:2}, (_,i)=>({id:`sf_${i+1}`,home:"",away:""}))},
  "3rd": {enabled:false, matches: [{id:"3rd_1",home:"",away:""}]},
  final: {enabled:false, matches: [{id:"final_1",home:"",away:""}]},
});
...
function emptyPred() {
  ...
  // knockoutMatches are dynamic — keys added as admin configures each round
  return p;
}
const emptyResults=()=>({groupMatches:{},knockoutMatches:{},champion:"",topScorer:"",goldenBall:"",bestGk:"",revelation:"",disappointment:""});
const emptyPredKo=()=>{const p={};KO_ROUNDS.forEach(r=>{for(let i=1;i<=r.matches;i++){p[`${r.id}_${i}`]={winner:""};}});return p;};
...
export default function App(){
  ...
  const [specialInputs,setSpecialInputs]=useState({topScorer:"",goldenBall:"",bestGk:""});
  ...
  const calcScoreDetail=(pred)=>{
    ...
    // Score knockout matches dynamically from koConfig
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"20px 16px"}}>
          ...
          <div>
            ...
            {!koConfig[selRound]?.enabled?(
              <div style={{textAlign:"center",padding:"40px 20px",background:"rgba(255,255,255,0.03)",borderRadius:12,border:"1px solid rgba(255,255,255,0.07)"}}>
                ...
                <div style={{fontSize:13,color:"#6a8aaa"}}>El Admin habilitará esta ronda cuando se conozcan los cruces reales</div>
              </div>
            ):(
              koConfig[selRound].matches.map(m=>{
                ...
                return(
                  <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:14,marginBottom:10,border:`1px solid ${r?.winner?"rgba(100,200,100,0.2)":"rgba(255,255,255,0.07)"}`}}>
                    <div style={{fontSize:12,color:"#aac8cc",marginBottom:8,fontWeight:600}}>{flag(m.home)} {m.home} vs {m.away} {flag(m.away)}</div>
                    {r?.winner&&<div style={{marginBottom:8,fontSize:11,color:"#80cc60"}}>✅ Pasó: {flag(r.winner)} {r.winner}</div>}
                    {pred?.winner&&<div style={{marginBottom:6,fontSize:11,color:"#f0d060"}}>Tu pick: {flag(pred.winner)} {pred.winner}</div>}
                    {!locked&&<div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                      {teams.map(t=><button key={t} onClick={()=>setPred("knockoutMatches",m.id,"winner",t)} style={{flex:1,background:pred?.winner===t?"linear-gradient(180deg,#c8a800,#a07800)":"rgba(255,255,255,0.06)",color:pred?.winner===t?"#0a1628":"#c8d8e8",border:pred?.winner===t?"none":"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"10px 6px",fontSize:12,cursor:"pointer",fontWeight:pred?.winner===t?700:400,textAlign:"center"}}>{flag(t)} {t}</button>)}
                    </div>}
                  </div>
                );
              })
        ...
        {tab==="admin"&&isAdmin&&(
          <div>
            ...
            <div style={{fontSize:11,color:"#6a8aaa",marginBottom:10}}>Define los cruces y habilita cada ronda cuando se conozcan los clasificados</div>
            <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
              {KO_ROUNDS.map(r=><button key={r.id} onClick={()=>setSelRound(r.id)} style={chip(selRound===r.id)}>{r.label}</button>)}
            </div>
            {/* Enable/disable toggle */}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:"rgba(255,255,255,0.04)",borderRadius:10,border:"1px solid rgba(255,255,255,0.07)",marginBottom:12}}>
              <span style={{fontSize:13,color:"#aac8cc",fontWeight:700}}>{koConfig[selRound]?.enabled?"🟢 Ronda habilitada":"🔴 Ronda bloqueada"}</span>
              <button onClick={()=>{
                const next={...koConfig,[selRound]:{...koConfig[selRound],enabled:!koConfig[selRound]?.enabled}};
                setKoConfig(next);
                saveData("ko_config",next);
              }} style={{background:koConfig[selRound]?.enabled?"rgba(255,80,80,0.2)":"rgba(80,200,80,0.2)",border:"none",borderRadius:8,padding:"6px 14px",color:koConfig[selRound]?.enabled?"#ff8888":"#80ff80",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                {koConfig[selRound]?.enabled?"🔒 Bloquear":"🔓 Habilitar"}
              </button>
            </div>
            {/* Define matchups */}
                ...
                <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px",marginBottom:8,border:"1px solid rgba(255,255,255,0.07)"}}>
                  ...
                  <div style={{display:"flex",gap:6,marginBottom:8,alignItems:"center"}}>
                    <select value={m.home||""} onChange={e=>{
                      ...
                      setKoConfig(next);saveData("ko_config",next);
                  ...
                  {m.home&&m.away&&<div style={{fontSize:11,color:"#aac8cc",marginBottom:6}}>Ganador real:</div>}
                  {m.home&&m.away&&<div style={{display:"flex",gap:8}}>
                    {[m.home,m.away].map(t=><button key={t} onClick={()=>updateResults(prev=>({...prev,knockoutMatches:{...prev.knockoutMatches,[m.id]:{winner:t}}}))} style={{flex:1,background:r.winner===t?"linear-gradient(135deg,#006620,#00aa40)":"rgba(255,255,255,0.05)",color:r.winner===t?"#fff":"#c8d8e8",border:r.winner===t?"none":"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"8px",fontSize:12,cursor:"pointer",fontWeight:r.winner===t?700:400}}>{flag(t)} {t}</button>)}
                  </div>}
                </div>
              );
            })}
            ...
            {[
              ...
              {key:"topScorer",label:"👟 Bota de Oro",options:STAR_PLAYERS,freeText:true,inputKey:"topScorer"},
              {key:"goldenBall",label:"🏅 Balón de Oro",options:STAR_PLAYERS,freeText:true,inputKey:"goldenBall"},
              {key:"bestGk",label:"🧤 Guante de Oro",options:STAR_GKS,freeText:true,inputKey:"bestGk"},
            ...
            ].map(({key,label,options,freeText,teamPick,inputKey})=>(
              <div key={key} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px",marginBottom:10,border:"1px solid rgba(255,255,255,0.07)"}}>
                ...
                {freeText&&inputKey&&<div style={{display:"flex",gap:6,marginTop:4}}><input value={specialInputs[inputKey]||""} onChange={e=>setSpecialInputs(prev=>({...prev,[inputKey]:e.target.value}))} placeholder="Otro nombre..." style={{flex:1,background:"#071828",border:"1px solid #1a4a6a",borderRadius:7,color:"#e8f4e8",fontSize:12,padding:"5px 8px",outline:"none"}}/><button onClick={()=>{const v=specialInputs[inputKey];if(v?.trim()){setSpecialResult(key,v.trim());setSpecialInputs(prev=>({...prev,[inputKey]:""}));}}} style={{background:"rgba(0,120,0,0.4)",border:"none",borderRadius:7,color:"#80ff80",padding:"5px 10px",fontSize:12,cursor:"pointer",fontWeight:700}}>OK</button></div>}
              </div>
            ))}
