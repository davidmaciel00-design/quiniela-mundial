...
const EMOJIS = ["🏆","⚽","🔥","🌟","🎯"];
const DEADLINE = new Date("2026-12-31T23:59:00");
  ...
  const calcScore = (pred) => {
    ...
    Object.values(koConfig).forEach(roundCfg => {
      ...
      roundCfg.matches.forEach(m => {
        ...
        if (p.winner === r.winner) knockout += PTS.knockoutWinner;
      });
    });
                  ...
                  <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:14,marginBottom:10,border:`1px solid ${r?.winner?"rgba(100,200,100,0.2)":"rgba(255,255,255,0.07)"}`}}>
                    ...
                    {!locked && teams.length===2 && (
                      <div style={{display:"flex",gap:8}}>
                        {teams.map(t => (
                          <button key={t} onClick={()=>setPred("knockoutMatches",m.id,"winner",t)}
                            style={{flex:1,background:pred?.winner===t?"linear-gradient(180deg,#c8a800,#a07800)":"rgba(255,255,255,0.06)",color:pred?.winner===t?"#0a1628":"#c8d8e8",border:pred?.winner===t?"none":"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"10px 6px",fontSize:12,cursor:"pointer",fontWeight:pred?.winner===t?700:400,textAlign:"center"}}>
                            {flag(t)} {t}
                          </button>
                        ))}
                      </div>
              ...
              return (
                <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px",marginBottom:8,border:"1px solid rgba(255,255,255,0.07)"}}>
                  ...
                  {m.home && m.away && (
                    <div style={{display:"flex",gap:8}}>
                      {[m.home,m.away].map(t => (
                        <button key={t} onClick={()=>setResult("knockoutMatches",m.id,"winner",t)}
                          style={{flex:1,background:r.winner===t?"linear-gradient(135deg,#006620,#00aa40)":"rgba(255,255,255,0.05)",color:r.winner===t?"#fff":"#c8d8e8",border:r.winner===t?"none":"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"8px",fontSize:12,cursor:"pointer",fontWeight:r.winner===t?700:400}}>
                          {flag(t)} {t}
                        </button>
                      ))}
                    </div>
