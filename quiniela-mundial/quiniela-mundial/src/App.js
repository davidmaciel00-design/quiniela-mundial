...
const DRAFT_TEAMS = {
  "Sergio":      ["Paraguay","Gan. Colombia/Ghana","México"],
  "Juan Carlos": ["Estados Unidos","Gan. Argentina/C.Verde","Gan. Bélgica/Senegal"],
  "David":       ["Gan. Australia/Egipto","Gan. Suiza/Argelia","Gan. España/Austria"],
  "Julio":       ["Inglaterra","Noruega","Francia"],
  "Miguel":      ["Canadá","Brasil","Gan. Portugal/Croacia"],
};
const DRAFT_ORPHAN = "Marruecos";
...
function emptyResults() {
  return {groupMatches:{},knockoutMatches:{},champion:"",topScorer:"",goldenBall:"",bestGk:"",revelation:"",disappointment:"",draftStatus:{}};
}
...
export default function App() {
  ...
  const updateMyPred = useCallback((updater, ignorelock=false) => {
    if (locked && !ignorelock) return;
    setMyPred(prev => {
  ...
  const setPred = (type, matchId, field, value) =>
    updateMyPred(prev => ({...prev, [type]: {...prev[type], [matchId]: {...(prev[type]?.[matchId]||{}), [field]: value}}}), type==="knockoutMatches");
  ...
  const calcScore = (pred) => {
    ...
    Object.values(koConfig).forEach(roundCfg => {
      ...
      roundCfg.matches.forEach(m => {
        ...
        const predWinner = ph>pa?m.home:pa>ph?m.away:null;
        if (ph===rh && pa===ra) {
          // Exact score
          knockout += PTS.groupExact;
          // If it was a draw and pred matches penalty winner too
          if (rh===ra && r.penaltyWinner && predWinner===null && p.penaltyWinner && p.penaltyWinner===r.penaltyWinner) {
            // already got 3, no extra
          }
        } else if (rh===ra && r.penaltyWinner) {
          // Real match was a draw, check if pred winner matches penalty winner
          if (predWinner === r.penaltyWinner) knockout += PTS.groupResult;
        } else if (getRes(rh,ra)!=="empate" && predWinner === getRes(rh,ra)) {
          knockout += PTS.groupResult;
        }
      });
    });
  ...
  const TABS = isAdmin
    ? [{id:"admin",label:"⚙️ Admin"},{id:"scores",label:"📊 Tabla"},{id:"draft",label:"🎱 Draft"}]
    : [{id:"scores",label:"📊 Tabla"},{id:"group",label:"📋 Grupos"},{id:"knockout",label:"🏆 Eliminatorias"},{id:"special",label:"⭐ Especiales"},{id:"draft",label:"🎱 Draft"},{id:"pin",label:"🔑 Mi PIN"}];
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0a1628 0%,#0d2137 40%,#0f2d1f 100%)",fontFamily:"'Trebuchet MS','Segoe UI',sans-serif",color:"#e8f4e8",paddingBottom:60}}>
      ...
      <div style={{background:"linear-gradient(90deg,#c8a800,#f0d060,#c8a800)",padding:"14px 20px 10px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          ...
          <div style={{fontSize:9,color:"#0a1628",opacity:0.4,letterSpacing:1}}>v2.0</div>
        </div>
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"20px 16px"}}>
          ...
          <div>
            ...
            {!koConfig[selRound]?.enabled ? (
              ...
              koConfig[selRound].matches.map(m => {
                ...
                const pred = myPred?.knockoutMatches?.[m.id] || {home:"",away:""};
                const r = results.knockoutMatches?.[m.id];
                const hasResult = r && r.home!==undefined && r.home!=="";
                return (
                  <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px",marginBottom:10,border:`1px solid ${hasResult?"rgba(100,200,100,0.2)":"rgba(255,255,255,0.07)"}`}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{flex:1,textAlign:"right",fontSize:13,fontWeight:600}}>{flag(m.home)} {m.home}</span>
                      <input type="number" min="0" max="20" value={pred.home} onChange={e=>setPred("knockoutMatches",m.id,"home",e.target.value)}
                        disabled={koConfig[selRound]?.votingLocked || hasResult}
                        style={{width:40,textAlign:"center",background:koConfig[selRound]?.votingLocked||hasResult?"#040e18":"#071828",border:"1px solid #1a4a6a",borderRadius:6,color:"#f0d060",fontSize:16,fontWeight:700,padding:4}}/>
                      <span style={{color:"#5a7a9a",fontWeight:700}}>-</span>
                      <input type="number" min="0" max="20" value={pred.away} onChange={e=>setPred("knockoutMatches",m.id,"away",e.target.value)}
                        disabled={koConfig[selRound]?.votingLocked || hasResult}
                        style={{width:40,textAlign:"center",background:koConfig[selRound]?.votingLocked||hasResult?"#040e18":"#071828",border:"1px solid #1a4a6a",borderRadius:6,color:"#f0d060",fontSize:16,fontWeight:700,padding:4}}/>
                      <span style={{flex:1,textAlign:"left",fontSize:13,fontWeight:600}}>{m.away} {flag(m.away)}</span>
                    </div>
                    {hasResult && <div style={{textAlign:"center",marginTop:6,fontSize:11,color:"#80cc60"}}>Resultado real: {r.home}-{r.away}</div>}
                    {koConfig[selRound]?.votingLocked && !hasResult && <div style={{textAlign:"center",marginTop:6,fontSize:11,color:"#ff8888"}}>🔒 Votación cerrada</div>}
                  </div>
                );
              })
            )}
        ...
        {tab==="draft" && (
          <div>
            <h2 style={{color:"#f0d060",fontSize:15,letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>🎱 Draft — Equipos de Octavos</h2>
            <div style={{color:"#6a8aaa",fontSize:12,marginBottom:16}}>El ganador del Mundial entre los 5 equipos gana el draft</div>

            {/* Orphan */}
            <div style={{background:"rgba(255,200,0,0.08)",border:"1px solid rgba(255,200,0,0.2)",borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:20}}>🏳️</span>
              <div>
                <div style={{fontSize:12,color:"#f0d060",fontWeight:700}}>Equipo huérfano: {DRAFT_ORPHAN}</div>
                <div style={{fontSize:11,color:"#6a8aaa"}}>Si tu equipo lo enfrenta y ganas esa serie, te lo quedas</div>
              </div>
            </div>

            {/* Player cards */}
            {PLAYERS.map((player, pi) => {
              const teams = DRAFT_TEAMS[player] || [];
              const statuses = results.draftStatus || {};
              const isMe = pi === myIdx && !isAdmin;
              const aliveCount = teams.filter(t => statuses[t] !== "eliminated").length;
              const hasChampion = teams.some(t => statuses[t] === "champion");
              return (
                <div key={player} style={{
                  background: isMe ? "linear-gradient(90deg,rgba(200,168,0,0.15),rgba(200,168,0,0.03))" : "rgba(255,255,255,0.04)",
                  borderRadius:10, padding:"12px 14px", marginBottom:10,
                  border: isMe ? "1px solid rgba(200,168,0,0.3)" : hasChampion ? "1px solid rgba(100,255,100,0.3)" : aliveCount===0 ? "1px solid rgba(255,80,80,0.2)" : "1px solid rgba(255,255,255,0.07)"
                }}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                    <span style={{fontSize:18}}>{EMOJIS[pi]}</span>
                    <span style={{fontSize:15,fontWeight:600,flex:1,color:"#e8f4e8"}}>{player} {isMe && <span style={{fontSize:10,color:"#c8a800"}}>← tú</span>}</span>
                    <span style={{fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20,
                      background: hasChampion ? "rgba(100,255,100,0.15)" : aliveCount===0 ? "rgba(255,80,80,0.15)" : "rgba(255,255,255,0.08)",
                      color: hasChampion ? "#80ff80" : aliveCount===0 ? "#ff8888" : "#aac8cc"
                    }}>
                      {hasChampion ? "🏆 Campeón" : aliveCount===0 ? "💀 Eliminado" : `${aliveCount} vivo${aliveCount!==1?"s":""}`}
                    </span>
                  </div>
                    ...
                    {teams.map(t => {
                      ...
                      return (
                        <div key={t} style={{
                          ...
                          background: status==="champion" ? "rgba(100,255,100,0.12)" : status==="eliminated" ? "rgba(255,80,80,0.08)" : "rgba(255,255,255,0.06)",
                          border: status==="champion" ? "1px solid rgba(100,255,100,0.3)" : status==="eliminated" ? "1px solid rgba(255,80,80,0.2)" : "1px solid rgba(255,255,255,0.1)",
                          opacity: status==="eliminated" ? 0.5 : 1
                        }}>
                          <div style={{fontSize:11,color:"#e8f4e8",fontWeight:600,lineHeight:1.3}}>{flag(t)}<br/>{t}</div>
            ...
            {/* Admin controls */}
            {isAdmin && (
              <div style={{marginTop:20,background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"14px",border:"1px solid rgba(255,255,255,0.07)"}}>
                <div style={{fontSize:13,fontWeight:700,color:"#f0d060",marginBottom:12}}>⚙️ Marcar estado de equipos</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {Object.entries(DRAFT_TEAMS).flatMap(([,teams])=>teams).concat([DRAFT_ORPHAN]).filter((t,i,a)=>a.indexOf(t)===i).map(t => {
                    const status = (results.draftStatus||{})[t];
                    return (
                      <div key={t} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,minWidth:80}}>
                        <div style={{fontSize:11,color:"#aac8cc",textAlign:"center",lineHeight:1.2}}>{flag(t)} {t}</div>
                        <div style={{display:"flex",gap:4}}>
                          {[{s:"",label:"🟢"},{s:"eliminated",label:"❌"},{s:"champion",label:"🏆"}].map(({s,label})=>(
                            <button key={s} onClick={()=>{
                              updateResults(prev=>({...prev,draftStatus:{...(prev.draftStatus||{}),[t]:s}}));
                            }} style={{background:status===s||(!status&&s==="")?"rgba(200,168,0,0.2)":"rgba(255,255,255,0.05)",border:status===s||(!status&&s==="")?"1px solid rgba(200,168,0,0.4)":"1px solid rgba(255,255,255,0.1)",borderRadius:6,padding:"3px 6px",fontSize:13,cursor:"pointer"}}>
                              {label}
                            </button>
                          ))}
                        </div>
            ...
            <div style={{background:"rgba(255,255,255,0.04)",borderRadius:10,border:"1px solid rgba(255,255,255,0.07)",marginBottom:12,overflow:"hidden"}}>
              <div style={{padding:"10px 14px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <div>
                    <div style={{fontSize:13,color:"#aac8cc",fontWeight:700}}>{koConfig[selRound]?.enabled?"🟢 Ronda visible":"🔴 Ronda oculta"}</div>
                    <div style={{fontSize:11,color:"#5a7a9a",marginTop:2}}>{koConfig[selRound]?.enabled?"Los jugadores ven esta ronda":"Los jugadores no ven esta ronda aún"}</div>
                  </div>
                  <button onClick={()=>{
                    const next = {...koConfig, [selRound]:{...koConfig[selRound],enabled:!koConfig[selRound]?.enabled}};
                    updateKoConfig(next);
                  }} style={{background:koConfig[selRound]?.enabled?"rgba(255,80,80,0.2)":"rgba(80,200,80,0.2)",border:"none",borderRadius:8,padding:"8px 14px",color:koConfig[selRound]?.enabled?"#ff8888":"#80ff80",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                    {koConfig[selRound]?.enabled?"🙈 Ocultar":"👁️ Mostrar"}
                  </button>
                </div>
                {koConfig[selRound]?.enabled && (
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingTop:8,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
                    <div>
                      <div style={{fontSize:13,color:"#aac8cc",fontWeight:700}}>{koConfig[selRound]?.votingLocked?"🔒 Votación cerrada":"✏️ Votación abierta"}</div>
                      <div style={{fontSize:11,color:"#5a7a9a",marginTop:2}}>{koConfig[selRound]?.votingLocked?"Los jugadores solo pueden ver sus picks":"Los jugadores pueden editar sus picks"}</div>
                    </div>
                    <button onClick={()=>{
                      const next = {...koConfig, [selRound]:{...koConfig[selRound],votingLocked:!koConfig[selRound]?.votingLocked}};
                      updateKoConfig(next);
                    }} style={{background:koConfig[selRound]?.votingLocked?"rgba(80,200,80,0.2)":"rgba(255,80,80,0.2)",border:"none",borderRadius:8,padding:"8px 14px",color:koConfig[selRound]?.votingLocked?"#80ff80":"#ff8888",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                      {koConfig[selRound]?.votingLocked?"🔓 Abrir votación":"🔒 Cerrar votación"}
                    </button>
                  </div>
                )}
              </div>
                ...
                <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px",marginBottom:8,border:"1px solid rgba(255,255,255,0.07)"}}>
                  ...
                  {m.home && m.away && (
                    <div style={{marginTop:8}}>
                      <div style={{fontSize:11,color:"#6a8aaa",marginBottom:6}}>Resultado real (a 120'):</div>
                      ...
                      {r.home!==undefined && r.home!=="" && r.away!==undefined && r.away!=="" && parseInt(r.home)===parseInt(r.away) && (
                        <div>
                          <div style={{fontSize:11,color:"#f0d060",marginBottom:6}}>⚽ Empate — ¿Quién ganó en penales?</div>
                          <div style={{display:"flex",gap:8}}>
                            {[m.home,m.away].map(t=>(
                              <button key={t} onClick={()=>updateResults(prev=>({...prev,knockoutMatches:{...prev.knockoutMatches,[m.id]:{...prev.knockoutMatches?.[m.id],penaltyWinner:t}}}))}
                                style={{flex:1,background:r.penaltyWinner===t?"linear-gradient(135deg,#006620,#00aa40)":"rgba(255,255,255,0.05)",color:r.penaltyWinner===t?"#fff":"#c8d8e8",border:r.penaltyWinner===t?"none":"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"8px",fontSize:12,cursor:"pointer",fontWeight:r.penaltyWinner===t?700:400}}>
                                {flag(t)} {t}
                              </button>
                            ))}
                          </div>
                          {r.penaltyWinner && <button onClick={()=>updateResults(prev=>({...prev,knockoutMatches:{...prev.knockoutMatches,[m.id]:{...prev.knockoutMatches?.[m.id],penaltyWinner:""}}}))}
                            style={{width:"100%",marginTop:4,background:"rgba(255,80,80,0.1)",border:"1px solid rgba(255,80,80,0.2)",borderRadius:6,padding:"4px",fontSize:11,color:"#ff8888",cursor:"pointer"}}>
                            ✕ Quitar ganador en penales
                          </button>}
                        </div>
                      )}
                    </div>
