              ...
              koConfig[selRound].matches.map(m => {
                ...
                return (
                  <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:14,marginBottom:10,border:`1px solid ${r?.winner?"rgba(100,200,100,0.2)":"rgba(255,255,255,0.07)"}`}}>
                    ...
                    {r?.winner && <div style={{marginBottom:8,padding:"6px 10px",background:"rgba(0,168,0,0.15)",borderRadius:7,fontSize:12,color:"#80cc60",fontWeight:700}}>✅ Pasó: {flag(r.winner)} {r.winner}</div>}
                    {pred?.winner && !r?.winner && <div style={{marginBottom:6,fontSize:11,color:"#f0d060"}}>Tu pick: {flag(pred.winner)} {pred.winner}</div>}
                    {!r?.winner && teams.length===2 && (
                      <div style={{display:"flex",gap:8,marginTop:6}}>
                        {teams.map(t => (
                          <button key={t}
                            ...
                            style={{flex:1,background:pred?.winner===t?"linear-gradient(180deg,#c8a800,#a07800)":"rgba(255,255,255,0.06)",color:pred?.winner===t?"#0a1628":"#c8d8e8",border:pred?.winner===t?"none":"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"12px 6px",fontSize:13,cursor:"pointer",fontWeight:pred?.winner===t?700:500,textAlign:"center"}}>
                            {flag(t)} {t}
                          </button>
                        ))}
                      </div>
            ...
            {koConfig[selRound]?.matches.map((m,idx) => {
              ...
              return (
                <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px",marginBottom:8,border:"1px solid rgba(255,255,255,0.07)"}}>
                  ...
                  {m.home && m.away && (
                    <div>
                      <div style={{fontSize:11,color:"#6a8aaa",marginBottom:6}}>Ganador real:</div>
                      <div style={{display:"flex",gap:8,marginBottom: r.winner?6:0}}>
                        {[m.home,m.away].map(t => (
                          <button key={t} onClick={()=>setResult("knockoutMatches",m.id,"winner",t)}
                            style={{flex:1,background:r.winner===t?"linear-gradient(135deg,#006620,#00aa40)":"rgba(255,255,255,0.05)",color:r.winner===t?"#fff":"#c8d8e8",border:r.winner===t?"none":"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"10px",fontSize:13,cursor:"pointer",fontWeight:r.winner===t?700:400}}>
                            {flag(t)} {t}
                          </button>
                        ))}
                      </div>
                      {r.winner && (
                        <button onClick={()=>setResult("knockoutMatches",m.id,"winner","")}
                          style={{width:"100%",background:"rgba(255,80,80,0.1)",border:"1px solid rgba(255,80,80,0.2)",borderRadius:6,padding:"5px",fontSize:11,color:"#ff8888",cursor:"pointer",marginTop:4}}>
                          ✕ Quitar resultado
                        </button>
                      )}
                    </div>
