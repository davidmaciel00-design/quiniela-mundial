...
const GROUPS = [
  { name: "A", teams: ["Qatar", "Ecuador", "Senegal", "Países Bajos"] },
  { name: "B", teams: ["Inglaterra", "Iran", "EE.UU.", "Gales"] },
  { name: "C", teams: ["Argentina", "Arabia Saudí", "México", "Polonia"] },
  { name: "D", teams: ["Francia", "Australia", "Dinamarca", "Túnez"] },
  { name: "E", teams: ["España", "Costa Rica", "Alemania", "Japón"] },
  { name: "F", teams: ["Bélgica", "Canadá", "Marruecos", "Croacia"] },
  { name: "G", teams: ["Brasil", "Serbia", "Suiza", "Camerún"] },
  { name: "H", teams: ["Portugal", "Ghana", "Uruguay", "Corea del Sur"] },
];
...
const KO_ROUNDS = [
  {id:"ro16",label:"Octavos"},{id:"qf",label:"Cuartos"},
  {id:"sf",label:"Semifinales"},{id:"3rd",label:"3er Puesto"},{id:"final",label:"Final"},
];
const KO_MATCHES = [
  {id:"ro16_1",round:"ro16",label:"1A vs 2B"},{id:"ro16_2",round:"ro16",label:"1C vs 2D"},
  {id:"ro16_3",round:"ro16",label:"1E vs 2F"},{id:"ro16_4",round:"ro16",label:"1G vs 2H"},
  {id:"ro16_5",round:"ro16",label:"2A vs 1B"},{id:"ro16_6",round:"ro16",label:"2C vs 1D"},
  {id:"ro16_7",round:"ro16",label:"2E vs 1F"},{id:"ro16_8",round:"ro16",label:"2G vs 1H"},
...
const FLAGS = {"Qatar":"🇶🇦","Ecuador":"🇪🇨","Senegal":"🇸🇳","Países Bajos":"🇳🇱","Inglaterra":"🏴󠁧󠁢󠁥󠁮󠁧󠁿","Iran":"🇮🇷","EE.UU.":"🇺🇸","Gales":"🏴󠁧󠁢󠁷󠁬󠁳󠁿","Argentina":"🇦🇷","Arabia Saudí":"🇸🇦","México":"🇲🇽","Polonia":"🇵🇱","Francia":"🇫🇷","Australia":"🇦🇺","Dinamarca":"🇩🇰","Túnez":"🇹🇳","España":"🇪🇸","Costa Rica":"🇨🇷","Alemania":"🇩🇪","Japón":"🇯🇵","Bélgica":"🇧🇪","Canadá":"🇨🇦","Marruecos":"🇲🇦","Croacia":"🇭🇷","Brasil":"🇧🇷","Serbia":"🇷🇸","Suiza":"🇨🇭","Camerún":"🇨🇲","Portugal":"🇵🇹","Ghana":"🇬🇭","Uruguay":"🇺🇾","Corea del Sur":"🇰🇷"};
const flag = t => FLAGS[t]||"🏳️";

const PLAYERS = ["Sergio","Juan Carlos","Iñigo","Gustavo","David","Julio"];
const PLAYER_PINS = ["1111","2222","3333","4444","5555","6666"];
...
function PickSelector({label,pts,value,onChange,options,freeText=false,teamPick=false}){
  const [custom,setCustom]=useState("");
  return(
    <div style={{background:"rgba(255,255,255,0.04)",borderRadius:12,padding:14,marginBottom:12,border:"1px solid rgba(255,255,255,0.08)"}}>
      ...
      {value&&<div style={{marginBottom:8,padding:"6px 10px",background:"rgba(200,168,0,0.15)",borderRadius:8,fontSize:13,color:"#f0d060",fontWeight:700}}>✅ {teamPick?flag(value):"⚽"} {value}<button onClick={()=>onChange("")} style={{marginLeft:8,background:"none",border:"none",color:"#888",cursor:"pointer",fontSize:11}}>✕</button></div>}
...
function LockScreen({onLogin}){
  ...
  const tryLogin=()=>{
    ...
    else if(selected!==null&&selected!==undefined){if(pin===PLAYER_PINS[selected]){onLogin("player",selected);return;}}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const init=async()=>{
      ...
      if(!isAdmin){
        const pred=await loadData(`pred_${myIdx}`);
        setMyPred(pred||emptyPred());
      } else {
        const preds=await Promise.all(PLAYERS.map((_,i)=>loadData(`pred_${i}`)));
        setAllPreds(preds.map((p,i)=>p||emptyPred()));
      }
      setLoading(false);
    };
    init();
  ...
  useEffect(()=>{
    ...
    const iv=setInterval(async()=>{
      const [res,...preds]=await Promise.all([loadData("results"),...PLAYERS.map((_,i)=>loadData(`pred_${i}`))]);
      if(res)setResults(res);
      setAllPreds(preds.map((p,i)=>p||emptyPred()));
  ...
  const calcScore=(pred)=>{
    ...
    MATCHES.forEach(m=>{
      ...
      if(ph===rh&&pa===ra)s+=PTS.groupExact;
      else if(res(ph,pa)===res(rh,ra))s+=PTS.groupResult;
    });
    KO_MATCHES.forEach(m=>{
      const r=results.knockoutMatches[m.id];const p=pred.knockoutMatches[m.id];
      if(!r?.winner||!p?.winner)return;
      if(p.winner===r.winner)s+=PTS.knockoutWinner;
    });
    ["champion","topScorer","goldenBall","bestGk","revelation","disappointment"].forEach(k=>{
      if(pred[k]&&results[k]&&pred[k]===results[k])s+=PTS[k];
    });
    return s;
  };

  const predsForScore=PLAYERS.map((_,i)=>i===myIdx&&!isAdmin?myPred:allPreds[i]);
  const ranked=PLAYERS.map((n,i)=>({name:n,score:calcScore(predsForScore[i]),idx:i})).sort((a,b)=>b.score-a.score);

  if(!session)return<LockScreen onLogin={(role,playerIdx)=>{setSession({role,playerIdx});setTab(role==="admin"?"admin":"group");}}/>;
  ...
  const TABS=isAdmin?[{id:"admin",label:"⚙️ Admin"},{id:"scores",label:"📊 Tabla"}]:[{id:"group",label:"📋 Grupos"},{id:"knockout",label:"🏆 Eliminatorias"},{id:"special",label:"⭐ Especiales"},{id:"scores",label:"📊 Tabla"}];
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0a1628 0%,#0d2137 40%,#0f2d1f 100%)",fontFamily:"'Trebuchet MS','Segoe UI',sans-serif",color:"#e8f4e8",paddingBottom:60}}>
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"20px 16px"}}>
        ...
        {tab==="group"&&!isAdmin&&(
          <div>
            <h2 style={{color:"#f0d060",fontSize:15,letterSpacing:2,textTransform:"uppercase",marginBottom:14}}>Tus pronósticos — Grupos</h2>
            <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
              {"ABCDEFGH".split("").map(g=><button key={g} onClick={()=>setSelGroup(g)} style={chip(selGroup===g)}>Grupo {g}</button>)}
            </div>
            {MATCHES.filter(m=>m.group===selGroup).map(m=>{
              const pred=myPred.groupMatches[m.id];
              return(
                <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px",marginBottom:10,border:"1px solid rgba(255,255,255,0.07)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{flex:1,textAlign:"right",fontSize:13,fontWeight:600}}>{flag(m.home)} {m.home}</span>
                    <input type="number" min="0" max="20" value={pred.home} onChange={e=>setPred("groupMatches",m.id,"home",e.target.value)} style={{width:40,textAlign:"center",background:"#071828",border:"1px solid #1a4a6a",borderRadius:6,color:"#f0d060",fontSize:16,fontWeight:700,padding:4}}/>
                    <span style={{color:"#5a7a9a",fontWeight:700}}>-</span>
                    <input type="number" min="0" max="20" value={pred.away} onChange={e=>setPred("groupMatches",m.id,"away",e.target.value)} style={{width:40,textAlign:"center",background:"#071828",border:"1px solid #1a4a6a",borderRadius:6,color:"#f0d060",fontSize:16,fontWeight:700,padding:4}}/>
        ...
        {tab==="knockout"&&!isAdmin&&(
          <div>
            <h2 style={{color:"#f0d060",fontSize:15,letterSpacing:2,textTransform:"uppercase",marginBottom:14}}>Tus pronósticos — Eliminatorias</h2>
            <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
            ...
            {KO_MATCHES.filter(m=>m.round===selRound).map(m=>{
              ...
              return(
                <div key={m.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:14,marginBottom:10,border:"1px solid rgba(255,255,255,0.07)"}}>
        ...
        {tab==="special"&&!isAdmin&&(
          <div>
            <h2 style={{color:"#f0d060",fontSize:15,letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>⭐ Picks Especiales</h2>
            <div style={{color:"#6a8aaa",fontSize:12,marginBottom:16}}>Se resuelven al final del torneo</div>
            <PickSelector label="🥇 Campeón del Mundial" pts={10} value={myPred.champion} onChange={v=>setSpecial("champion",v)} options={ALL_TEAMS} teamPick/>
            <PickSelector label="👟 Bota de Oro" pts={8} value={myPred.topScorer} onChange={v=>setSpecial("topScorer",v)} options={STAR_PLAYERS} freeText/>
            <PickSelector label="🏅 Balón de Oro" pts={6} value={myPred.goldenBall} onChange={v=>setSpecial("goldenBall",v)} options={STAR_PLAYERS} freeText/>
            <PickSelector label="🧤 Guante de Oro" pts={5} value={myPred.bestGk} onChange={v=>setSpecial("bestGk",v)} options={STAR_GKS} freeText/>
            <PickSelector label="🌟 Equipo Revelación" pts={6} value={myPred.revelation} onChange={v=>setSpecial("revelation",v)} options={ALL_TEAMS} teamPick/>
            <PickSelector label="😬 Equipo Decepción" pts={5} value={myPred.disappointment} onChange={v=>setSpecial("disappointment",v)} options={ALL_TEAMS} teamPick/>
          </div>
        )}
          ...
          return(
            <div>
              <div style={{background:"linear-gradient(135deg,rgba(200,168,0,0.2),rgba(200,168,0,0.05))",border:"1px solid rgba(200,168,0,0.4)",borderRadius:12,padding:"14px 16px",marginBottom:16,textAlign:"center"}}>
                ...
                <div style={{fontSize:11,color:"#7a9a7a",marginTop:2}}>${PRIZE_PER_PLAYER} por jugador · 6 participantes</div>
              </div>
              ...
              {ranked.map((p,rank)=>{
                ...
                return(
                  <div key={p.idx} style={{display:"flex",alignItems:"center",gap:12,background:isMe?"linear-gradient(90deg,rgba(200,168,0,0.18),rgba(200,168,0,0.04))":isWinning?"linear-gradient(90deg,rgba(200,168,0,0.10),rgba(0,0,0,0))":"rgba(255,255,255,0.04)",borderRadius:10,padding:"14px 16px",marginBottom:8,border:isMe?"1px solid rgba(200,168,0,0.35)":isWinning?"1px solid rgba(200,168,0,0.2)":"1px solid rgba(255,255,255,0.06)"}}>
              ...
              <div style={{marginTop:8,padding:"10px 12px",background:"rgba(255,255,255,0.04)",borderRadius:10,border:"1px solid rgba(255,255,255,0.07)",fontSize:12,color:"#6a8aaa"}}>🔒 Pronósticos privados · 🔄 Tabla se actualiza cada 30s</div>
            </div>
          );
        })()}
        ...
        {tab==="admin"&&isAdmin&&(
          <div>
            <h2 style={{color:"#f0d060",fontSize:15,letterSpacing:2,textTransform:"uppercase",marginBottom:16}}>⚙️ Resultados Reales</h2>
